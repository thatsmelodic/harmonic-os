...
create policy if not exists "cms_copy_public_read" on public.cms_copy for select using (true);
create policy if not exists "cms_copy_auth_write" on public.cms_copy for update using (auth.role() = 'authenticated');

create policy if not exists "assets_public_read" on public.assets for select using (true);
create policy if not exists "assets_auth_insert" on public.assets for insert with check (auth.role() = 'authenticated');

create policy if not exists "seasons_public_read" on public.seasons for select using (true);
create policy if not exists "seasons_auth_update" on public.seasons for update using (auth.role() = 'authenticated');
...
create policy if not exists "public_read_brand_assets" on storage.objects for select using (bucket_id in ('brand-assets', 'world-media', 'shop-assets', 'beats', 'community-uploads'));
create policy if not exists "authenticated_upload_assets" on storage.objects for insert with check (auth.role() = 'authenticated');
