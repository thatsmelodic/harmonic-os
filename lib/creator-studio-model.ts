...
export function createStudioSections(world: WorldKey): StudioSection[] {
  return worldSections[world].map(([id, label, kind], order) => ({ id, label, kind, visible: true, order }));
}
...
export function parseStudioSections(raw: string | undefined, world: WorldKey): StudioSection[] {
  ...
  try {
    ...
    const allowed = new Set(defaults.map((section) => section.id));
    const valid = parsed.filter((section) => allowed.has(section.id));
