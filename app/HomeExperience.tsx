...
export default function HomeExperience() {
  ...
  return (
    <main className={styles.page} style={theme}>
      {config.backgroundMedia && config.backgroundMediaKind === 'video' && <video className={styles.managedBackground} src={config.backgroundMedia} autoPlay loop muted playsInline />}
      {config.backgroundMedia && config.backgroundMediaKind !== 'video' && <img className={styles.managedBackground} src={config.backgroundMedia} alt="" />}
      {config.backgroundMedia && <div className={styles.managedOverlay} aria-hidden="true" />}
      <div className={styles.noise} aria-hidden="true" />
      <header className={styles.topbar}>
        <button className={styles.brand} type="button" onClick={() => setOpenWorld(null)}>
          {config.logoMedia ? <img className={styles.brandImage} src={config.logoMedia} alt="" /> : <span className={styles.brandMark}>∞</span>}<span>{config.brandName}</span>
        </button>
