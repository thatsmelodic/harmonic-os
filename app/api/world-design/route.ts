...
export async function GET() {
  try {
    const rows = await supabaseRest<Array<{ world: string; settings: unknown }>>({
      path: 'world_builder_designs',
      query: 'select=world,settings',
    });
    return NextResponse.json({ designs: Object.fromEntries(rows.map((row) => [row.world, row.settings])) });
  } catch (error) {
...
export async function PUT(request: Request) {
  ...
  await supabaseRest({
    path: 'world_builder_designs',
    method: 'POST',
    query: 'on_conflict=world',
    prefer: 'resolution=merge-duplicates,return=minimal',
    body: { world: body.world, settings: body.settings, updated_at: new Date().toISOString() },
  });
