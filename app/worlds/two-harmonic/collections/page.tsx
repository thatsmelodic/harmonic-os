...
export default function TwoHarmonicCollectionsPage() {
  ...
  return (
    <main className="two-harmonic-world relative min-h-screen overflow-hidden px-4 py-8 pb-28 sm:px-6" data-world-shell>
      ...
      <div className="absolute inset-0 -z-30" style={{ background: 'radial-gradient(circle at 25% 15%,rgba(245,239,228,.18),transparent 24rem),radial-gradient(circle at 78% 45%,rgba(111,90,69,.22),transparent 28rem),linear-gradient(145deg,#17120e,#2a2119 50%,#0d0a08)' }} />
          ...
          <article className="overflow-hidden rounded-[3rem] border" style={{ borderColor: '#8c785f', background: 'linear-gradient(145deg,rgba(45,36,28,.95),rgba(14,11,9,.98))', boxShadow: '0 0 90px rgba(216,199,170,.1)' }}>
            <div className="relative grid min-h-[560px] place-items-center overflow-hidden p-8 text-center">
              ...
              <div className="relative z-10 max-w-3xl">
                {view === 'front' && <><GarmentReveal key={active.slug} symbol={active.symbol} name={active.name} mark={active.slug.includes('frequency-zip')} /><p className="mt-7 text-xs font-black uppercase tracking-[.26em] text-[#b9a78c]">{active.color} · {active.type}</p><h2 className="mt-3 text-5xl font-black tracking-[-.07em] text-[#f5efe4] sm:text-7xl">{active.name}</h2><p className="mx-auto mt-4 max-w-2xl text-base leading-8 text-[#c8b99f]">{active.detail}</p></>}
                {view === 'detail' && <><p className="text-8xl">🪡</p><h2 className="mt-6 text-5xl font-black tracking-[-.07em] text-[#f5efe4]">Material carries memory.</h2><p className="mx-auto mt-5 max-w-2xl text-xl font-black text-[#d8c7aa]">{active.material}</p><p className="mx-auto mt-5 max-w-2xl text-base leading-8 text-[#b9a78c]">{active.fit}. Every trim, seam, weight, contrast panel, zipper, and placement must justify a luxury price under close inspection.</p></>}
                {view === 'story' && <><img src="/identity/melody-cup-neon.svg" alt="Pour Your Frequency melody mark" className="mx-auto h-32 w-32 object-contain drop-shadow-[0_0_28px_rgba(192,132,252,.35)]" /><p className="mt-6 text-xs font-black uppercase tracking-[.26em] text-[#b9a78c]">The melody inside this garment</p><blockquote className="mx-auto mt-4 max-w-2xl text-4xl font-black leading-tight tracking-[-.05em] text-[#f5efe4]">“{active.lyric}”</blockquote><p className="mx-auto mt-5 max-w-xl text-sm leading-7 text-[#b9a78c]">The product, campaign, packaging, and Lift U Up experience all resolve into the same emotional frequency.</p></>}
              </div>
            </div>
            <div className="grid gap-3 border-t border-[#8c785f]/45 p-6 sm:grid-cols-3"><div className="rounded-2xl border border-[#d8c7aa]/15 bg-black/20 p-4"><p className="text-xs uppercase tracking-[.16em] text-[#a9967a]">Private Preview</p><p className="mt-2 text-xl font-black text-[#f5efe4]">${active.price}</p></div><div className="rounded-2xl border border-[#d8c7aa]/15 bg-black/20 p-4"><p className="text-xs uppercase tracking-[.16em] text-[#a9967a]">Connected Melody</p><p className="mt-2 text-xl font-black text-[#f5efe4]">Lift U Up</p></div><Link href={`/worlds/two-harmonic/collections/${active.slug}`} className="grid place-items-center rounded-2xl bg-[#f5efe4] p-4 text-center text-sm font-black text-[#241b14]">Enter Garment Room →</Link></div>
          </article>
        </section>
      </div>
    </main>
