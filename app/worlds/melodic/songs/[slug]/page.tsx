      ...
      <div className="mx-auto max-w-7xl">
        ...
        <section className="grid gap-6 lg:grid-cols-[1.05fr_.95fr]">
          ...
          <aside className="rounded-[2.8rem] border border-purple-200/15 bg-black/50 p-6 shadow-purple-glow backdrop-blur-2xl">
            <div className="grid aspect-square place-items-center rounded-[2rem] border border-white/10" style={{ background: `radial-gradient(circle, ${song.colors[0]}55, ${song.colors[1]}22 50%, transparent 70%)` }}><div className="text-center"><div className="mx-auto h-36 w-36 animate-pulse rounded-full border border-purple-100/25 bg-white/[.04] shadow-[0_0_80px_rgba(183,108,255,.35)]" /><p className="mt-5 font-mono text-xs uppercase tracking-[.22em] text-white/35">Frequency visualizer</p></div></div>
            <div className="mt-5"><MelodicAudioPlayer title={song.title} src={song.audioUrl} accent={song.colors[0]} /></div>
            <button className="mt-3 w-full rounded-full border border-white/10 bg-white/[.04] px-4 py-3 text-sm font-black text-white/65">Save Memory</button>
          </aside>
        </section>
