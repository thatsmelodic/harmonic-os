...
const rooms = [
  ...
  { href: '/worlds/schmackinn/map', icon: '🗺️', label: 'Flavor City', description: 'Explore districts by culture, mood, value, craving, and occasion.' },
...
export function SchmackinnWorldExperience() {
  ...
  return (
    <main className="schmackinn-world relative isolate min-h-screen overflow-hidden pb-28">
      ...
      <section className="harmonic-container py-8 sm:py-12">
        <nav className="mb-8 flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap gap-2">
            <Link href="/worlds/schmackinn/reviews" className="rounded-full bg-purple-300 px-4 py-3 text-sm font-black text-black transition hover:bg-purple-200">Review Theater</Link>
            <Link href="/worlds/schmackinn/map" className="rounded-full border border-purple-200/20 bg-purple-300/10 px-4 py-3 text-sm font-black text-purple-100 transition hover:bg-purple-300/20">Flavor City</Link>
            <Link href="/worlds/schmackinn/flavor-lab" className="rounded-full border border-white/10 bg-white/[.04] px-4 py-3 text-sm font-black text-white/70">Flavor Lab</Link>
            <Link href="/worlds/schmackinn/memories" className="rounded-full border border-white/10 bg-white/[.04] px-4 py-3 text-sm font-black text-white/70">Memories</Link>
          </div>
        </nav>
        ...
        <div className="grid gap-8 lg:grid-cols-[1.05fr_.95fr] lg:items-center">
          <div className="glass-panel schmackinn-panel rounded-[2.5rem] p-6 sm:p-10">
            ...
            <p className="mt-6 max-w-2xl text-base leading-8 text-white/64 sm:text-lg">Enter reviews like scenes, explore the city by appetite, decode why a plate hit, and preserve the meals that became real memories.</p>
      ...
      <section className="harmonic-container py-8"><div className="mb-5"><p className="text-xs font-black uppercase tracking-[.34em] text-white/40">World Rooms</p><h2 className="mt-3 text-3xl font-black tracking-[-.06em] sm:text-5xl">Choose how you enter the food scene.</h2></div><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{rooms.map((room) => <Link key={room.href} href={room.href} className="group rounded-[2rem] border border-white/10 bg-black/35 p-5 backdrop-blur-xl transition hover:-translate-y-2 hover:border-purple-200/30 hover:bg-purple-300/[.06]"><span className="text-4xl">{room.icon}</span><h3 className="mt-5 text-2xl font-black tracking-[-.05em]">{room.label}</h3><p className="mt-3 text-sm leading-6 text-white/50">{room.description}</p><p className="mt-5 text-sm font-black text-purple-100">Enter Room →</p></Link>)}</div></section>
