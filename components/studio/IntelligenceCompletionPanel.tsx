...
export function IntelligenceCompletionPanel({ world, onApplyEvent }: { world: HarmonicWorldId; onApplyEvent?: (patch: Partial<HarmonicEngineState>, label: string) => void }) {
  ...
  function toggleEvent(id: string) {
    setApprovedEventIds((current) => current.includes(id) ? current.filter((eventId) => eventId !== id) : [...current, id]);
  }
  ...
  return (
    <article className="rounded-[2rem] border border-purple-200/20 bg-[linear-gradient(135deg,rgba(183,108,255,.10),rgba(0,0,0,.35))] p-5 backdrop-blur-2xl">
      <p className="text-xs font-black uppercase tracking-[.28em] text-purple-100/50">Completion Sprint</p>
      <h3 className="mt-3 text-3xl font-black tracking-[-.06em]">Memory, Explain Why, and Event Engine</h3>
      <p className="mt-2 max-w-3xl text-sm leading-7 text-white/58">These systems make Brain V3 more trustworthy: it remembers creator preferences, explains suggestions, and builds event triggers that still require approval.</p>
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-3">
        ...
        <section className="rounded-2xl border border-white/10 bg-black/25 p-4">
          ...
          <div className="mt-4 grid gap-3">
            {events.map((event) => {
              const approved = approvedEventIds.includes(event.id);
              return (
                <div key={event.id} className="rounded-xl border border-white/10 bg-white/[.035] p-3">
                  <div className="flex flex-wrap items-start justify-between gap-3"><strong className="text-sm text-white/75">{event.label}</strong><button type="button" onClick={() => toggleEvent(event.id)} className="rounded-full border border-purple-200/20 px-3 py-1 text-xs font-black text-purple-100/70">{approved ? 'Approved' : 'Approve'}</button></div>
