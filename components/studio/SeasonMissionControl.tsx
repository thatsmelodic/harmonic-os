...
export function SeasonMissionControl() {
  ...
  function changeSeason(nextSeason: SeasonKey) {
    setSeason(seasonDefaults[nextSeason]);
  }

  function patch(patchState: Partial<SeasonState>) {
    setSeason((current) => ({ ...current, ...patchState }));
  }
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-3 max-w-3xl text-sm leading-7 text-orange-50/62">
            Seasons sit above FX and audio. They decide weather, light, wildlife, NPC energy, AI instincts, commerce opportunities, and how each world expresses the same time of year.
          </p>
        </div>
        <div className="rounded-2xl border border-orange-200/15 bg-black/30 p-4 text-right">
          <p className="font-mono text-xs uppercase tracking-[.22em] text-white/35">Active</p>
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-[.9fr_1.1fr]">
        <div className="grid gap-5">
          ...
          <SeasonPanel title="Autumn Detail System" eyebrow="Flagship Controls">
            ...
            <button type="button" onClick={() => setSeason((current) => applySeasonEmotion(current, 'chaos pressure'))} className="rounded-full border border-orange-200/20 px-5 py-3 text-sm font-black text-orange-100/75 hover:bg-orange-200/10">
        ...
        <div className="grid gap-5 content-start">
          <article className="relative overflow-hidden rounded-[2.5rem] border border-orange-200/10 bg-black/35 p-5 backdrop-blur-2xl">
            <div
              ...
              style={{
                background: `radial-gradient(circle at 30% 15%, ${season.palette.glow}, transparent 26rem), radial-gradient(circle at 82% 25%, ${season.palette.accent}, transparent 22rem), ${season.palette.shadow}`,
              }}
            />
          ...
          <article className="rounded-[2rem] border border-white/10 bg-black/35 p-5 backdrop-blur-2xl">
            ...
            <div className="mt-4 grid gap-2 sm:grid-cols-2">
              {season.aiInstincts.map((instinct) => (
                <div key={instinct} className="rounded-2xl border border-white/10 bg-white/[.045] p-4 text-sm font-bold leading-6 text-white/62">{instinct}</div>
              ))}
            </div>
          </article>
        </div>
