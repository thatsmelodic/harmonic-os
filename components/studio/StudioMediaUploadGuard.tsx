...
export function StudioMediaUploadGuard({children}:{children:ReactNode}){
 ...
 async function persist(file:File){
  const id=crypto.randomUUID(),preview=URL.createObjectURL(file);setItems(current=>[...current,{id,name:file.name,preview,status:'preview',progress:0}]);
  const temporary=await findTemporary(file.name);const world=temporary?.world??selectedWorldFromStudio();
  try{
   if(file.size>MAX_BYTES)throw new Error('This file is larger than the 5 GB upload limit.');
   setItems(current=>current.map(item=>item.id===id?{...item,status:'uploading',message:'Preparing secure resumable upload…'}:item));
