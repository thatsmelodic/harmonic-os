...
export function StudioMediaUploadGuard({children}:{children:ReactNode}){
 ...
 async function persist(file:File){
  ...
  try{
   const secret=window.sessionStorage.getItem(SECRET_KEY)||window.localStorage.getItem(SECRET_KEY)||'';if(!secret)throw new Error('Enter the Studio Secret once in the top toolbar. It will stay saved on this device for future uploads.');
   ...
   setItems(current=>current.map(item=>item.id===id?{...item,status:'ready',preview:data.url,message:`Uploaded to ${world} and autosaving…`}:item));setTimeout(()=>setItems(current=>current.filter(item=>item.id!==id)),5000);
