...
function selectedWorldFromStudio():WorldKey{
 const active=Array.from(document.querySelectorAll('button')).find((button)=>button.className.includes('bg-white/12'));
 const label=active?.querySelector('p')?.textContent?.trim().toLowerCase()||'';
 if(label==='homepage')return'home';
 if(label==='melodic')return'melodic';
 if(label==='fried em')return'fried-em';
 if(label==='schmackinn')return'schmackinn';
 if(label==='2 harmonic')return'two-harmonic';
 if(label==='global system')return'global';
 return'schmackinn';
}

export function StudioMediaUploadGuard({children}:{children:ReactNode}){
 const {settings,addMedia,updateMedia,removeMedia}=useWorldCustomization();
 ...
 async function persist(file:File){
  const id=crypto.randomUUID(),preview=URL.createObjectURL(file);setItems(current=>[...current,{id,name:file.name,preview,status:'preview'}]);await new Promise(resolve=>setTimeout(resolve,0));setItems(current=>current.map(item=>item.id===id?{...item,status:'uploading'}:item));
  const temporary=await findTemporary(file.name);const world=temporary?.world??selectedWorldFromStudio();
  try{
   const secret=window.sessionStorage.getItem(SECRET_KEY)||'';if(!secret)throw new Error('Open Publishing, enter the Studio Secret, then upload again.');
   const form=new FormData();form.append('file',file);form.append('world',world);
   const response=await fetch('/api/world-design/upload',{method:'POST',headers:{'x-harmonic-studio-key':secret},body:form});const data=await response.json().catch(()=>({}));if(!response.ok||!data.url)throw new Error(data.error||'Upload failed.');
   if(temporary){updateMedia(world,temporary.asset.id,{url:data.url,kind:data.type==='video'?'video':'image',visible:true});}
   else{addMedia(world,{name:file.name,url:data.url,kind:data.type==='video'?'video':'image',placement:'hero',section:'hero',x:50,y:50,width:100,opacity:100,rotation:0,zIndex:2,loop:true,muted:true,fit:'cover',visible:true,locked:false});}
   setItems(current=>current.map(item=>item.id===id?{...item,status:'ready',preview:data.url,message:`Uploaded, added to ${world}, and ready to save`}:item));setTimeout(()=>setItems(current=>current.filter(item=>item.id!==id)),5000);
