...
const routes = [
  { position: [0, 6.8, 25] as const, lookAt: [0, 3.4, 2] as const },
  { position: [0, 7.5, 13] as const, lookAt: [0, 3.2, -14] as const },
  { position: [-16, 5.8, 7] as const, lookAt: [-12.5, 2.2, -9] as const },
  { position: [16, 5.4, 7] as const, lookAt: [12, 2, -7] as const },
  { position: [0, 9.8, -5] as const, lookAt: [0, 2, -31] as const },
];
...
function Scene(props: TwoHarmonicCanvasProps) {
  return <>
    ...
    <directionalLight position={[18, 24, 14]} intensity={3.4} color="#ffd08a" castShadow={props.quality === 'cinematic'} shadow-mapSize-width={2048} shadow-mapSize-height={2048} />
...
export function TwoHarmonicBeigeDynastyCanvas(props: TwoHarmonicCanvasProps) {
  return <div aria-hidden="true" style={{ position: 'absolute', inset: 0, zIndex: 0 }}>
    <Canvas dpr={props.quality === 'cinematic' ? [1, 1.65] : 1} camera={{ position: [0, 6.8, 25], fov: 44, near: 0.1, far: 180 }} shadows={props.quality === 'cinematic'} gl={{ antialias: props.quality === 'cinematic', alpha: false, powerPreference: 'high-performance' }}>
